...
const int ServoPin = 9 ; // pin that connects to Servo motor (other servo wires connect to power)
const int TriggerPin = 2 ; //middle pin on ultasonic sensor
const int EchoPin = 4 ; // middle pin on ultrasonic sensor
// IR Detector Pins
const int RIGHT = A0;
const int MIDDLE = A1;
const int LEFT = A2;
...
const int numLine = 2; // 2-8
const int distVal = 14; //5-25
...
void setup() {
  ...
  servo.write (0); //this integer is an angle on a scale from 0 (complete left) to 180 (complete right)
...
void loop() {
  ...
  // TODO: Get an initial IR reading
    ...
    //swivel servo
    int angle = map(blackline, 0, 1, 0, 90);
    servo.write(angle);
  ...
  if (globalCounter >= numLine) {
    ...
    break;
      ...
      // stops the function after have counted numLine lines
  }
  servo.write(90);
